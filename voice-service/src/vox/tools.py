...
Each handler:
  ...
  - returns a JSON STRING the Agent verbalizes back to the user

ElevenLabs' ClientToolResult protocol requires the `result` field to be a
string, so every handler returns json.dumps(...) — never a raw dict.
"""
...
class Tools:
    ...
    async def dispatch_command(self, params: dict) -> str:
